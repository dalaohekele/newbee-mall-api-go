...
func (mallUserService *MallUserService) CreateMallUser(mallUser manage.MallUser) (err error) {
	err = global.GVA_DB.Create(&mallUser).Error
	return err
}

// DeleteMallUser 删除MallUser记录
func (mallUserService *MallUserService) DeleteMallUser(mallUser manage.MallUser) (err error) {
	err = global.GVA_DB.Delete(&mallUser).Error
	return err
}

// DeleteMallUserByIds 批量删除MallUser记录
func (mallUserService *MallUserService) DeleteMallUserByIds(ids request.IdsReq) (err error) {
	err = global.GVA_DB.Delete(&[]manage.MallUser{}, "id in ?", ids.Ids).Error
	return err
}

// LockUser 修改用户状态
func (mallUserService *MallUserService) LockUser(ids request.IdsReq, lockStatus int) (err error) {
...
func (mallUserService *MallUserService) GetMallUser(id uint) (err error, mallUser manage.MallUser) {
	err = global.GVA_DB.Where("id = ?", id).First(&mallUser).Error
	return
}

// GetMallUserInfoList 分页获取商城注册用户列表
func (mallUserService *MallUserService) GetMallUserInfoList(info manageReq.MallUserSearch) (err error, list interface{}, total int64) {
	limit := info.PageSize
